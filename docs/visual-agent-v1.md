...
先用guides show design-tasks按用途选择方法。品牌、产品、海报、插画、空间与非舞台视频不加载VJ配方；“主视觉”“循环”等词不单独触发舞台流程。单次任务无需project init。持续的非舞台项目兼容现有档案格式，stage字段留空，不要求舞台资料。
...
使用 guides list 得到本地资源列表，guides show <id>读取内容；无认证、无远程请求。运行时根据设计任务加载所需指南，不一次读取所有内容。安装包内resources.json固定摘要，models route的guideInfo定位包内资源，不依赖当前工作目录。
...
3. 适配为项目所需指南，复核与任务用途或用户审美冲突的建议；新增指南同时更新guideRegistry和Skill入口。
