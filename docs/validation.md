...
运行npm run check验证类型、构建、资源完整性和行为测试。实际tgz安装测试从不同工作目录读取指南、确认随包安装 wowidea、easyai 与 GPT Image 2.5 提示词三个 Skill、验证每份模型指南在包内与安装目录都可读，并验证自定义Skill与项目资料保留。审批变更失效、单任务恢复、防重复、H3素材顺序及未支持组合有回归覆盖。
