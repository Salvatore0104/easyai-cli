...
Seedance follows [approval](seedance-gate.md), manifest and cost preflight. At <=200 points no extra cost question follows valid storyboard approval; above 200 obtain additional cost consent. Unknown Seedance cost cannot be assumed low. Other models need no unsolicited cost gate; an explicit user budget still requires its quote/max-cost flow.
...
Report pointsUsage.actualPoints from task-level billing only. If not_reported, explicitly say actual points were not returned; never substitute estimates, tokens, balance changes or zero. Save task/settings/references/outputs/QC in project records when used. Inspect media before quality claims. Partial completion remains partial.
