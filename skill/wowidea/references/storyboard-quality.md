...
参考驱动的第一格默认直接使用用户选定的原图字节，不让图片模型重新绘制。需要表现后续变化时，生成独立关键帧并保留来源角色；若要 2×2 contact sheet，在本地把原图和真实关键帧排版组合。不要要求模型一次生成整张多格板来证明连续性，这容易重构机位、地标和构图。contact sheet 只用于审阅，除非用户明确指定，不得把整张分镜板当作最终视频的首帧或唯一参考。

## 实际看图后评审

把来源图和候选分镜同时打开，逐项写出观察，不能只复述提示词或声称“保持一致”。以下任一项出现即 `verdict: revise`：

- 主体、建筑、产品、人物身份或关键地标被替换、移动或重新设计；
- 镜头位置、地平线、透视或构图关系明显改变，而用户没有批准改变；
- 第一格不能自然接上来源图，或各格不像同一场景的连续时刻；
- 出现提示词禁止的文字、标签、水印、边框、人物、物件或视觉风格；
- 关键动作读不清，时间推进重复，因果顺序错误，或特效遮蔽了叙事主体；
- 画面虽技术正确但过于模板化、廉价、杂乱、塑料感或缺乏明确视觉主次。
