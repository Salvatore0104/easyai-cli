...
Official editing/extension availability does not prove an EasyAI mapping. Check exact model, supported modes, reference limits, audio combinations and duration in the live catalogue and adapter. All Seedance generation follows [storyboard review](seedance-gate.md), the final manifest and `watermark:false`; stable public references are sent directly through the platform CLI. MinIO is optional only for an explicitly upload-only platform operation. A prompt-writing request alone makes no paid call.
