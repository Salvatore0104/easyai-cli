...
2. 先制作分镜表与分镜图。预计 ≤200 积分时不等待分镜批准，直接继续；预计 >200 时展示图片、设置与预计积分并取得一次明确确认。用户的新积分门槛取代旧的逐次审批规则。
...
6. `seedance payload manifest.json --file request.json` 导出最终私密请求，再 video preflight --file 获取服务器报价。≤200 积分时保留 storyboard_ready，不调用 approve，不提确认问题，直接 video generate --manifest --quote --idempotency-key，CLI 自动绑定分镜字节与请求。
7. 仅 >200 积分时一次展示分镜、设置、参考及报价并请用户确认；确认后调用 `seedance approve manifest.json --confirmation "I APPROVE STORYBOARD"`，通过 video generate --manifest --quote --yes --max-cost --idempotency-key 执行。不要再问第二次付费确认。任何创意/设置变化重新准备 manifest 与报价，仅新报价 >200 时再确认。未知费用不能假定低于门槛。
