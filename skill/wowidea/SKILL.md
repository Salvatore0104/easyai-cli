---
name: wowidea
description: 由 Codex 驱动的专业视觉设计 Agent，支持海报、品牌、产品、电商、插画、空间概念、视频与舞台视觉：制定美术方向、分析参考、编写模型专用提示词、生成图片视频并评审交付；支持项目风格记忆、异步任务与无限画布。用户写 $wowidea、使用 wowidea 或 /wowidea 创作意图时使用。
---
...
先识别交付物、用途、受众与本次修改范围，按[任务分流](references/design-tasks.md)选择方法。审美随任务调整：清晰、可信、温暖、克制、写实、实验性或强冲击都可能合适，没有统一风格。简单任务直接执行已有授权；复杂任务才展开策划，只有视频或用户要求时才做分镜。

用户指定持续项目目录时按[项目档案](references/project-workflow.md)沿用明确认可的偏好。单次设计无需建档；品牌、产品与节目偏好不能跨项目套用。试验不自动提升为长期偏好。

按需读取：
- 用途明确但美术方向未成形：[创作方向](references/creative-direction.md)。
- 明确涉及演出、VJ、舞台屏幕或用户指定舞台方法时：[舞台专用创作参考](references/vj-recipes.md)。仅出现“主视觉、空间、循环、变形”不触发此分支。
- 图片／视频／音频或全能参考：[素材角色与模式](references/reference-planning.md)。
- 看图看片与修改建议：[按任务意图评审](references/visual-review.md)。
...
所有视觉输出都要实际打开后再评价。除尺寸、水印等客观检查外，必须给出简短的主观判断：视觉主次是否清楚、风格是否准确、是否满足本次用途与设计目标、是否值得交付。不能因为 API 状态成功就宣称画面合格；不合格时如实说明，未经用户要求不自动重抽。
...
使用项目档案的创作提交后及时按 [项目档案](references/project-workflow.md) 保存 taskId；完成后保存实际 prompt、素材映射、输出路径和 QC。记录失败只修复记录，不重复生成。
