...
export interface SeedanceManifest {
  ...
  mode: string; lastFrameRequested: boolean; storyboard: Array<{ shot: number; description: string; image: string }>;
...
export function creativeHash(m: SeedanceManifest): string {
  return payloadHash({ prompt: m.prompt, model: m.model, duration: m.duration, aspectRatio: m.aspectRatio, resolution: m.resolution, audio: m.audio, watermark: m.watermark, mode: m.mode, lastFrameRequested: m.lastFrameRequested, storyboard: m.storyboard, referenceCounts: m.referenceCounts, references: m.references.map(({ type, role, localPath, url, objectKey, sha256 }) => ({ type, role, localPath, url, objectKey, sha256 })) });
}
export async function validateManifest(m: SeedanceManifest, verifyRemote = false): Promise<void> {
  ...
  for (const shot of m.storyboard) {
    ...
    } else await stat(shot.image).catch(() => { throw new CliError(`Storyboard image not found for shot ${shot.shot}: ${shot.image}`, ExitCode.Approval); });
  }
...
export function seedancePayload(m: SeedanceManifest): Record<string, unknown> {
  ...
  return { prompt: m.prompt, model: m.model, duration: m.duration, aspect_ratio: m.aspectRatio, resolution: m.resolution, audio: m.audio, watermark: false, mode: m.mode, last_frame: m.lastFrameRequested,
