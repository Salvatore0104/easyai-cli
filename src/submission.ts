...
export async function submitImageWithRecovery(api: EasyAiApi, payload: Record<string, unknown>, idempotencyKey: string): Promise<unknown> {
  try { return await api.post("/v1/images/generations", payload, { "Idempotency-Key": idempotencyKey }); }
  catch (error) {
    ...
    } catch (lookupError) { throw uncertainImageSubmission(idempotencyKey, error, lookupError); }
    throw uncertainImageSubmission(idempotencyKey, error);
  }
}

function uncertainImageSubmission(key: string, original: unknown, lookup?: unknown): CliError {
  const suffix = lookup ? ` Task lookup also failed: ${redact((lookup as Error).message || lookup)}` : " Task lookup returned no matching task.";
  return new CliError(`Image submission outcome is uncertain for idempotency key ${key}; no retry was made.${suffix} Original error: ${redact((original as Error).message || original)}`, ExitCode.Service);
}
