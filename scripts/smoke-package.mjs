...
const archive = resolve(process.argv[2] || 'easyai-cli-0.3.2.tgz');
...
try {
  ...
  assert.equal(run('guides', 'show', 'minimax-h3').version, '0.3.2');
  for (const id of ['design-tasks', 'production-rounds', 'recipe-authoring']) {
    ...
    assert.equal(guide.version, '0.3.2');
  }
  ...
  assert.equal(install().version, '0.3.2');
